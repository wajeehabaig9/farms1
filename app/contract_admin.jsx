...
import { Text, Image, TouchableOpacity, View, Button, TextInput, useEffect } from "react-native";
...
const ContractAdmin = () => {
    ...
    const [contractName, setContractName] = useState('MILIMILI');
    const [contractType, setContractType] = useState('Maize supply contract');
    const [contractNumber, setContractNumber] = useState('FNKECO123456');
        ...
        <SafeAreaView className="bg-white h-full p-2">
            ...
            <ScrollView>
                ...
                <View className="flex-row flex-wrap justify-between -mt-3">
                    <TouchableOpacity
                        className="w-[48%]"
                        onPress={() => router.push("/")} // Use the correct page path
                    >
                    ...
                    <View className="absolute bottom-7 left-4 w-full">
                        {isEditing ? (
                            <TextInput
                                ...
                                className="font-inter font-bold text-[16px] leading-[16px] text-white w-full min-h-[20px] p-0"
                            />
                        ) : (
                            <Text className="font-inter font-bold text-[16px] leading-[16px] text-white">{contractName}</Text>
                        )}
                    </View>

                    <View className="absolute flex-row items-center bottom-7 right-1 space-x-1">
                    ...
                    <View className="absolute bottom-2 left-4 w-full">
                        {isEditing ? (
                            <TextInput
                                ...
                                className="font-inter font-medium text-[12px] leading-[16px] text-white w-full min-h-[20px] p-0"
                            />
                        ) : (
                            <Text className="font-inter font-medium text-[12px] leading-[16px] text-white">{contractType}</Text>
                        )}
                    </View>

                    <View className="absolute bottom-2 right-2 w-full">
                        {isEditing ? (
                            <TextInput
                                value={contractNumber}
                                onChangeText={setContractNumber}
                                className="font-inter font-normal text-[10px] leading-[18px] text-white text-right min-h-[20px] p-0"
                            />
                        ) : (
                            <Text className="font-inter font-normal text-[10px] leading-[18px] text-white text-right">
                                {contractNumber}
                ...
                <View>
                    ...
                    {isEditing ? (
                        <TextInput
                            ...
                            onBlur={() => setIsEditing(false)} // Save changes when user clicks outside
                            style={{
                                fontSize: 12,
                                lineHeight: 16,
                                fontWeight: 'normal',
                        ...
                        <TouchableOpacity onPress={() => setIsEditing(true)}>
                            <Text style={{ fontSize: 12, lineHeight: 16, fontWeight: 'normal' }}>
                                {paragraph}
                            </Text>
                        </TouchableOpacity>
                    )}
